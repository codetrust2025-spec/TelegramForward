...
Recovery work does not waive branch protection. Recovery branches still require the protected `main` Pull Request and required passing checks; independent approval is optional for this single-owner repository. If review occurs, resolve every conversation before merge. Any emergency protection change must be separately authorized, time-bounded, logged, restored immediately, and reviewed afterward.
