...
function $8({
  ...
  return <div className="cand-proofs"><div className="cand-proofs-header"><span className="cand-field-label">Payment proofs<span className="cand-proofs-count">{t.length}</span></span>{!_ && t.length > 0 && <span className="cand-proofs-hint">Click a thumbnail to enlarge · drag to reorder is coming soon</span>}</div>{_ ? <div className="cand-proofs-empty cand-proofs-empty--blocked"><strong>Save the candidate first</strong>, then re-open this form to attach payment screenshots.</div> : <s.Fragment><div className={`cand-proofs-drop${g ? " cand-proofs-drop--active" : ""}${n ? " cand-proofs-drop--busy" : ""}`} onDragOver={b => {
      ...
      }}><input ref={m} type="file" accept="image/*" multiple={true} onChange={k} hidden={true} disabled={n} /><div className="cand-proofs-drop-icon" aria-hidden={true}>📷</div><div className="cand-proofs-drop-text">{n ? <strong>Uploading…</strong> : <s.Fragment><strong>Click or drop screenshots</strong><span className="cand-proofs-drop-sub">PNG · JPG · WebP · up to 8 MB each · select multiple</span></s.Fragment>}</div></div><input className="cand-input cand-input--small" placeholder="Optional note for the next upload — e.g. ₹10k UPI · 26 May" value={c} onChange={b => o(b.target.value)} disabled={n} /></s.Fragment>}{i && <div className="cand-proofs-error">{i}</div>}{aiResult && <div className="cand-proofs-ai-result" style={{margin:"8px 0",padding:"8px 12px",borderRadius:"6px",background:aiResult.verified?"rgba(34,197,94,.1)":"rgba(251,191,36,.1)",border:aiResult.verified?"1px solid rgba(34,197,94,.25)":"1px solid rgba(251,191,36,.25)",fontSize:"12px",lineHeight:"1.4"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"6px"}}><strong style={{color:aiResult.verified?"#22c55e":"#f59e0b",fontSize:"12px",whiteSpace:"nowrap"}}>{aiResult.verified?"✓ Verified":"⚠ Pending"}</strong><span style={{color:"rgba(255,255,255,.85)",fontSize:"12px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{aiResult.amount>0&&<span>₹{aiResult.amount.toLocaleString("en-IN")}</span>}{aiResult.utr_number&&<span> · UTR {aiResult.utr_number}</span>}{aiResult.payment_app&&<span> · {aiResult.payment_app}</span>}{aiResult.status&&aiResult.status!=="unknown"&&<span> · {aiResult.status}</span>}{aiResult.payment_date&&<span> · {aiResult.payment_date}</span>}</span><button type="button" onClick={()=>setAiResult(null)} style={{background:"none",border:"none",cursor:"pointer",opacity:.5,fontSize:"13px",color:"inherit",padding:"0 2px",flexShrink:0}}>×</button></div>{aiResult.narrative&&<div style={{marginTop:"6px",paddingTop:"6px",borderTop:"1px solid rgba(255,255,255,.08)",fontSize:"12px",color:"rgba(226,232,240,.9)",lineHeight:"1.5",fontStyle:"italic"}}>{aiResult.narrative}</div>}</div>}{t.length > 0 && <ul className="cand-proofs-grid">{t.map(b => <li className="cand-proof-card" key={b.id}><button type="button" className="cand-proof-thumb" onClick={() => d(b)} aria-label="Preview proof"><img src={`${ve}${b.url}`} alt={b.note || b.original_name || "payment proof"} loading="lazy" /></button><div className="cand-proof-meta">{f === b.id ? <div className="cand-proof-note-edit"><input className="cand-input cand-input--small" value={x} onChange={A => v(A.target.value)} placeholder="e.g. ₹10k UPI" autoFocus={true} /><button type="button" className="cand-btn cand-btn--xs cand-btn--primary" onClick={() => E(b)}>Save</button><button type="button" className="cand-btn cand-btn--xs cand-btn--ghost" onClick={() => h(null)}>Cancel</button></div> : <button type="button" className="cand-proof-note" onClick={() => {
            h(b.id);
            v(b.note || "");
...
function ResumeAutoFill({ onExtracted }) {
...
function X8({
  ...
  return <div className="cand-modal-backdrop" onClick={C => C.target === C.currentTarget && (t == null ? undefined : t())}><form className="cand-modal" onSubmit={M}><header className="cand-modal-header"><h3 className="cand-modal-title">{e ? "Edit candidate" : "Add candidate"}</h3><button type="button" className="cand-modal-close" onClick={t} aria-label="Close">×</button></header>{!e && <ResumeAutoFill onExtracted={(data) => { if (data.candidate_name && !l.name) g("name", data.candidate_name); if (data.technology && !l.technology) g("technology", data.technology); if (data.phone && !l.phone) g("phone", data.phone); }} />}<div className="cand-modal-body"><div className="cand-panel cand-panel--left"><h4 className="cand-panel-title">CANDIDATE DETAILS</h4><label className="cand-field"><span className="cand-field-label">Candidate name *</span><input ref={v} className="cand-input" value={l.name} onChange={C => g("name", C.target.value)} placeholder="e.g. NIKHIL" required={true} /></label><label className="cand-field"><span className="cand-field-label">Stage</span><select className="cand-input" value={l.stage} onChange={C => g("stage", C.target.value)}>{V8.map(C => <option value={C.value} key={C.value}>{C.label}</option>)}</select></label><label className="cand-field"><span className="cand-field-label">Technology</span><select className="cand-input" value={H8.includes(l.technology) ? l.technology : "_other_"} onChange={C => { if (C.target.value === "_other_") { g("technology", ""); } else { g("technology", C.target.value); } }}><option value="">— Select —</option>{H8.map(C => <option value={C} key={C}>{C}</option>)}<option value="_other_">Other…</option></select>{!H8.includes(l.technology) && l.technology !== "" && <input className="cand-input" value={l.technology} onChange={C => g("technology", C.target.value)} placeholder="Type technology" style={{marginTop:"4px"}} />}</label><label className="cand-field"><span className="cand-field-label">Phone</span><input className="cand-input" type="tel" value={l.phone} onChange={C => g("phone", C.target.value.replace(/[^\d+]/g, ""))} placeholder="9876543210" /></label><label className="cand-field"><span className="cand-field-label">Reference</span><ReferencePicker value={l.reference} onChange={C => g("reference", C)} options={refOpts} readOnly={a} placeholder="Handler name" title={a ? "Your handler name is set automatically" : undefined} /><span className="cand-field-hint">Earns 50% commission.</span></label><label className="cand-field"><span className="cand-field-label">Date</span><input className="cand-input" type="date" value={l.date} onChange={C => g("date", C.target.value)} /></label>{l.service_type === "profile_service" && <label className="cand-field"><span className="cand-field-label">% on CTC</span><input className="cand-input" type="number" min="0" max="100" step="0.5" value={l.ctc_percentage} onChange={C => g("ctc_percentage", C.target.value)} placeholder="e.g. 8.33" /></label>}<div className="cand-field cand-service-field"><span className="cand-field-label">Service type</span><div className="cand-service-options"><label className={`cand-service-card${l.service_type === "profile_service" ? " cand-service-card--on" : ""}`}><input type="radio" name="cand-service-type" checked={l.service_type === "profile_service"} onChange={() => m("profile_service")} /><span className="cand-service-card-body"><strong>Profile</strong><em>₹{Cu.toLocaleString("en-IN")}</em></span></label><label className={`cand-service-card${l.service_type === "round_wise" ? " cand-service-card--on" : ""}`}><input type="radio" name="cand-service-type" checked={l.service_type === "round_wise"} onChange={() => m("round_wise")} /><span className="cand-service-card-body"><strong>Round-wise</strong><em>₹{wi.toLocaleString("en-IN")}/₹{ki.toLocaleString("en-IN")}</em></span></label></div></div>{l.service_type === "round_wise" && <div className="cand-field cand-service-scope"><span className="cand-field-label">Scope</span><div className="cand-service-scope-options"><label className={`cand-service-scope-pill${!Eu(l.interview_scope) ? " cand-service-scope-pill--on" : ""}`}><input type="radio" name="cand-interview-scope" checked={!Eu(l.interview_scope)} onChange={() => _("external")} />External (regular round) · ₹{wi.toLocaleString("en-IN")}</label><label className={`cand-service-scope-pill${Eu(l.interview_scope) ? " cand-service-scope-pill--on" : ""}`}><input type="radio" name="cand-interview-scope" checked={Eu(l.interview_scope)} onChange={() => _("internal")} />Internal (joined org) · ₹{ki.toLocaleString("en-IN")}</label></div></div>}{l.service_type === "profile_service" && <label className={`cand-field cand-consultancy-field${l.consultancy ? " cand-consultancy-field--on" : ""}`}><span className="cand-field-label">Channel</span><div className="cand-consultancy-toggle"><input type="checkbox" id="cand-consultancy-cb" checked={!!l.consultancy} onChange={C => y(C.target.checked)} /><label htmlFor="cand-consultancy-cb" className="cand-consultancy-label"><span className="cand-consultancy-pip" aria-hidden={true} /><span className="cand-consultancy-text"><strong>{l.consultancy ? "Consultancy" : "Direct"}</strong><em>₹{N_(l.consultancy).toLocaleString("en-IN")}</em></span></label></div></label>}<div className="cand-field cand-bgv-field" style={l.service_type === "round_wise" ? {display:"none"} : undefined}><label className={`cand-consultancy-field${l.bgv_certificates ? " cand-consultancy-field--on" : ""}`}><span className="cand-field-label">Additional services</span><div className="cand-consultancy-toggle"><input type="checkbox" checked={!!l.bgv_certificates} onChange={C => B(C.target.checked)} /><span className="cand-consultancy-label"><span className="cand-consultancy-pip" /><span className="cand-consultancy-text"><strong>BGV certificates</strong><em>₹30,000 added to expected</em></span></span></div></label></div></div><div className="cand-panel cand-panel--right"><h4 className="cand-panel-title">PAYMENT & FOLLOW-UP</h4><label className="cand-field"><span className="cand-field-label">Expected ₹</span><input className="cand-input" type="number" min="0" step="500" value={l.expected_payment} onChange={C => g("expected_payment", C.target.value)} placeholder={String(os(l.service_type, l.consultancy, l.interview_scope))} /></label><label className="cand-field"><span className="cand-field-label">Received ₹</span><input className="cand-input" type="number" min="0" step="500" value={l.payment} onChange={C => g("payment", C.target.value)} placeholder="0" /></label><div className="cand-field"><span className={`cand-pay-status cand-pay-status--${E}`}>{E === "paid" && <s.Fragment>✓ Paid ({$n(k)})</s.Fragment>}{E === "partial" && <s.Fragment>● {$n(k)}/{$n(T)} · <strong>{$n(S)} pending</strong></s.Fragment>}{E === "unpaid" && <s.Fragment>○ {$n(T)} pending</s.Fragment>}</span>{k > 0 && (l.reference || "").trim() && <span className="cand-pay-handler-share">↻ {l.reference.trim()} earns {$n(wl(k, T, P))}</span>}</div>{b && <label className="cand-field cand-field--required"><span className="cand-field-label">Follow-up / Remark<span className="cand-field-required-tag">balance {$n(S)}</span></span><textarea className="cand-input cand-input--textarea" rows={2} value={l.follow_up} onChange={C => g("follow_up", C.target.value)} placeholder="Why is balance pending?" required={true} /></label>}<div className="cand-field"><$8 candidateId={e == null ? undefined : e.id} proofs={o} onChange={u} /></div></div></div>{h && <div className="cand-modal-error">{h}</div>}<footer className="cand-modal-footer"><button type="button" className="cand-btn cand-btn--ghost" onClick={t} disabled={d}>Cancel</button><button type="submit" className="cand-btn cand-btn--primary" disabled={d}>{d ? "Saving…" : e ? "Save changes" : "Add candidate"}</button></footer></form></div>;
}
function cr(e) {
  const t = Number(e) || 0;
  if (t === 0) {
...
function _Component26({
  ...
  const allTimePerformers = (allStats == null ? undefined : allStats.top_performers) || [];
  // Never blank: if the selected month has no attributed performers, fall back to all-time.
  const usingFallback = scopedPerformers.length === 0 && allTimePerformers.length > 0;
  const f = w.useMemo(() => {
  ...
  if (!f.length && !v) {
    ...
    return <section className="cand-top-perf"><header className="cand-top-perf-header"><div><h3 className="cand-top-perf-title">{p}{v && <span className="cand-top-perf-scope-tag">{v}</span>}</h3><p className="cand-top-perf-sub">{c ? <s.Fragment>{o ? <strong>{o}</strong> : "Your leads"}{v ? <s.Fragment> · <strong>{v}</strong></s.Fragment> : null} — your revenue and commission only.</s.Fragment> : v ? <s.Fragment>Scoped to <strong>{v}</strong> — {usingFallback ? <s.Fragment>no closes yet · showing <strong>all-time</strong> performers.</s.Fragment> : "ranked by completed revenue."}</s.Fragment> : <s.Fragment>All time — ranked by completed revenue.</s.Fragment>}{g && <s.Fragment> · <strong>{g.total}</strong> {g.label} · <strong>{$a(g.company)}</strong> company{g.client > g.company && <s.Fragment> · {$a(g.client)} client</s.Fragment>}{g.completed > 0 && <s.Fragment> · <strong>{$a(g.completed)}</strong> company closed</s.Fragment>}</s.Fragment>}</p></div><div className="cand-top-perf-controls">{i && !c && <button type="button" className="cand-btn cand-btn--primary cand-top-perf-earn-btn" onClick={i} title="Open a full board with every handler's salary, commission, paid out and net owed — with chart view"><span aria-hidden={true} style={{
              marginRight: 6
...
function CandidatesPanelImpl() {
  ...
  return <div className="cand-page"><header className="cand-header"><div className="cand-header-titles"><h2 className="cand-title">Candidates</h2></div></header><nav className="cand-tabs" role="tablist" aria-label="Candidates sections"><button type="button" role="tab" aria-selected={candTab === "overview"} className={`cand-tabs__btn${candTab === "overview" ? " cand-tabs__btn--active" : ""}`} onClick={() => setCandTab("overview")}>Overview</button><button type="button" role="tab" aria-selected={candTab === "candidates"} className={`cand-tabs__btn${candTab === "candidates" ? " cand-tabs__btn--active" : ""}`} onClick={() => setCandTab("candidates")}>Candidates</button><button type="button" role="tab" aria-selected={candTab === "performers"} className={`cand-tabs__btn${candTab === "performers" ? " cand-tabs__btn--active" : ""}`} onClick={() => setCandTab("performers")}>Earnings</button></nav><div className="cand-toolbar" role="region" aria-label="Candidate filters"><input className="cand-input cand-input--search" placeholder="Search name, tech, reference, phone, notes…" value={E} onChange={ge => b(ge.target.value)} /><select className="cand-input" value={service} onChange={ge => setService(ge.target.value)} aria-label="Filter by service type"><option value="all">All types</option><option value="profile_service">Profile</option><option value="round_wise">Round-wise</option></select><select className="cand-input" value={m} onChange={ge => _(ge.target.value)} aria-label="Filter by month">{Le.map(ge => <option value={ge.value} key={ge.value}>{ge.label.replace(' · this month', '')}</option>)}</select><select className="cand-input" value={g} onChange={ge => p(ge.target.value)}>{dR.map(ge => <option value={ge.value} key={ge.value}>{ge.label}</option>)}</select>{a && <select className={`cand-input${T !== "all" ? " cand-input--active" : ""}`} value={T} onChange={ge => S(ge.target.value)} aria-label="Filter by handler / reference" title="Show only candidates referred by this handler">{st.map(ge => <option value={ge.value} key={ge.value}>{ge.label}</option>)}</select>}<label className={`cand-toggle${y ? " cand-toggle--on" : ""}${(c == null ? undefined : c.pending_count) > 0 ? " cand-toggle--has-pending" : ""}`} title="Show only candidates with a pending balance"><input type="checkbox" checked={y} onChange={ge => k(ge.target.checked)} /><span>Pending only</span>{(c == null ? undefined : c.pending_count) > 0 && <span className="cand-toggle-badge">{c.pending_count}</span>}</label><button type="button" className="cand-btn cand-btn--ghost cand-btn--sm" onClick={() => setRo(true)} title="View all in-progress candidates grouped by technology">☷ Active list</button><button type="button" className="cand-btn cand-btn--ghost cand-btn--sm" onClick={() => triggerRosterDownload({ month: "all", reference: T })} title="Download CSV of all active (in-progress) candidates">⇩ Download active CSV</button>{a && <button type="button" className="cand-btn cand-btn--ghost cand-btn--sm" onClick={() => setShowExpenditure(true)} title="View total company expenditure — handler payouts + operational costs">📊 Total expenditure</button>}<button type="button" className="cand-btn cand-btn--primary cand-btn--sm" onClick={q}>+ Add candidate</button></div>{candTab === "overview" && c && <><J8 stats={c} scopeLabel={$e} onPayoutsClick={pe} handlerView={n} handlerName={t} scopeReference={T !== "all" ? T : n ? t : null} /></>}{candTab === "performers" && c && <EarningsBreakdown stats={c} allStats={u} month={m} onMonthChange={_} monthOptions={Le} onEditPayout={a ? me : undefined} handlerView={n} handlerName={t} formatCurrency={Jc} apiBase={ve} />}{x && <div className="cand-error">{x}</div>}{candTab === "candidates" && <div className="cand-table-wrap"><table className="cand-table"><thead><tr><th>Name</th><th>Service type</th><th>Technology</th><th>Stage</th><th>Payment</th><th>Date</th><th>Phone</th>{a && <th>Reference</th>}<th>Resume</th><th aria-label="Actions" /></tr></thead><tbody>{f && i.length === 0 ? <tr><td colSpan={a ? 9 : 8} className="cand-table-empty">Loading…</td></tr> : i.length === 0 ? <tr><td colSpan={a ? 9 : 8} className="cand-table-empty">No candidates match these filters. <button type="button" className="cand-link" onClick={q}>Add one</button>.</td></tr> : i.slice().sort((a2, b2) => { const da = a2.logged_date || a2.date || ''; const db = b2.logged_date || b2.date || ''; return db.localeCompare(da); }).map(ge => {
    ...
    }} onClose={() => G(false)} onChanged={fe} />}{P && a && <_Component30 handler={P} onClose={() => j(null)} onChanged={fe} />}{B && <_Component31 candidate={B} onClose={() => Z(null)} onEdit={ge => I(ge)} />}<CandidatesActiveRoster open={ro} onClose={() => setRo(false)} reference={T} />{showExpenditure && <CompanyExpenditure onClose={() => setShowExpenditure(false)} apiBase={ve} />}<_Component32 open={!!W} title={W == null ? undefined : W.title} message={W == null ? undefined : W.message} onVerified={W == null ? undefined : W.onVerified} onCancel={H} /></div>;
}
