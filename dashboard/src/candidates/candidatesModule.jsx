...
function _Component29({
  ...
  return <div className="cand-modal-backdrop" onClick={P => P.target === P.currentTarget && (r == null ? undefined : r())}><div className="cand-modal cand-modal--xl cand-modal--payout-wide"><header className="cand-modal-header cand-modal-header--payout"><div className="cand-modal-header-left"><h3 className="cand-modal-title">Manage handler payouts</h3><p className="cand-modal-sub cand-payout-bar"><span className="cand-payout-chunk"><strong>{Y.count}</strong> entr{Y.count === 1 ? "y" : "ies"}</span><span className="cand-payout-chunk cand-payout-chunk--earn" title="Auto-computed: 50% with shortfall penalty when client paid below prescribed tariff"><span className="cand-payout-pip" /> Owed (50%) <strong>{Jc(O)}</strong></span><span className="cand-payout-chunk cand-payout-chunk--ded" title="Every row in the payout ledger"><span className="cand-payout-pip" /> Paid out <strong>{Jc(M)}</strong></span><span className={`cand-payout-chunk ${C > 0 ? "cand-payout-chunk--net-pos" : C === 0 ? "cand-payout-chunk--net-zero" : "cand-payout-chunk--net-neg"}`}><span className="cand-payout-pip" />{C > 0 ? "Still owe " : C === 0 ? "Settled " : "Overpaid by "}<strong>{Jc(Math.abs(C))}</strong></span></p></div><div className="cand-modal-header-right">{(v !== "all" || p !== "all") && <button type="button" className="cand-btn cand-btn--ghost cand-btn--xs" onClick={() => {
            g("all");
            m("all");
          }}>Clear filters</button>}<button type="button" className="cand-modal-close" onClick={r} aria-label="Close">×</button></div></header><div className="cand-modal-body cand-modal-body--stack"><div className="cand-payout-form-grid"><form className="cand-allexp-form cand-exp-form--payout" onSubmit={ee}><label className="cand-field"><span className="cand-field-label">Handler *</span><select className="cand-input" value={k.reference} onChange={P => T(j => ({
              ...j,
              reference: P.target.value
            }))} required={true}><option value="">Select handler</option>{J.map(P => <option value={P} key={P}>{P}</option>)}</select></label><label className="cand-field"><span className="cand-field-label">Amount (₹) *<span className="cand-exp-kind-tag cand-exp-kind-tag--payout">subtracted from what's owed</span></span><input className="cand-input" type="number" min="0" step="100" value={k.amount} onChange={P => T(j => ({
              ...j,
