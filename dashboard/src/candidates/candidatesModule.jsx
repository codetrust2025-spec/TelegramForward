...
<div className="cand-payout-top-row">
<form className="cand-payout-form-card" onSubmit={ee}>
<h4 className="cand-payout-form-title">{_ ? "Edit payout" : "Log a new payout"}</h4>
<div className="cand-payout-form-grid">
<label className="cand-field"><span className="cand-field-label">Handler *</span><input className="cand-input" value={k.reference} onChange={P => T(j => ({...j, reference: P.target.value}))} placeholder="e.g. Thrilok" list="cand-allexp-ref-list" required={true} /><datalist id="cand-allexp-ref-list">{J.map(P => <option value={P} key={P} />)}</datalist></label>
<label className="cand-field"><span className="cand-field-label">Amount (₹) *<span className="cand-exp-kind-tag cand-exp-kind-tag--payout">subtracted from what's owed</span></span><input className="cand-input" type="number" min="0" step="100" value={k.amount} onChange={P => T(j => ({...j, amount: P.target.value}))} placeholder="5000" required={true} /></label>
<label className="cand-field"><span className="cand-field-label">Category *</span><select className="cand-input" value={k.category} onChange={P => T(j => ({...j, category: P.target.value}))}>{B0.map(P => <option value={P.value} key={P.value}>{P.label}</option>)}</select></label>
<label className="cand-field"><span className="cand-field-label">Date *</span><input className="cand-input" type="date" value={k.date} onChange={P => T(j => ({...j, date: P.target.value}))} /></label>
</div>
<label className="cand-field"><span className="cand-field-label">Note</span><input className="cand-input" value={k.note} onChange={P => T(j => ({...j, note: P.target.value}))} placeholder="e.g. May commission · taxi to client meeting" /></label>
{!_ && <div className="cand-payout-screenshot-row"><label className="cand-field cand-field--grow"><span className="cand-field-label">Payment screenshot (required)</span><div className={`cand-proof-upload-zone${proofFile ? " cand-proof-upload-zone--has-file" : ""}`} onClick={() => proofInputRef.current && proofInputRef.current.click()} role="button" tabIndex={0} onKeyDown={P => { if (P.key === "Enter" || P.key === " ") { proofInputRef.current && proofInputRef.current.click(); } }}><input ref={proofInputRef} type="file" accept="image/*" onChange={P => { const file = P.target.files && P.target.files[0]; if (file) { if (!/^image\//.test(file.type || "")) { f("Only image files are allowed (jpg / png / webp / gif / heic)"); return; } if (file.size > 8 * 1024 * 1024) { f("File too large (max 8 MB)"); return; } setProofFile(file); f(""); } }} hidden={true} />{proofFile ? <span className="cand-proof-upload-name">📷 {proofFile.name}</span> : <span className="cand-proof-upload-placeholder">🖼 Click to attach payment screenshot</span>}</div></label><button type="submit" className="cand-btn cand-btn--primary cand-payout-submit-btn" disabled={S || (!_ && !proofFile)}>{S ? "Saving…" : "+ Log payout"}</button></div>}
{_ && <div className="cand-payout-screenshot-row"><button type="button" className="cand-btn cand-btn--ghost" onClick={G}>Cancel edit</button><button type="submit" className="cand-btn cand-btn--primary cand-payout-submit-btn" disabled={S}>{S ? "Saving…" : "Save changes"}</button></div>}
</form>
<div className="cand-payout-filters-card">
<h4 className="cand-payout-form-title">Filters</h4>
<label className="cand-field"><span className="cand-field-label">Period</span><select className="cand-input" value={v} onChange={P => g(P.target.value)} aria-label="Filter by period">{Z.map(P => <option value={P.value} key={P.value}>{P.label}</option>)}</select></label>
<label className="cand-field"><span className="cand-field-label">Category</span><select className="cand-input" value={p} onChange={P => m(P.target.value)} aria-label="Filter by category"><option value="all">All categories</option>{B0.map(P => <option value={P.value} key={P.value}>{P.label}</option>)}</select></label>
{(h !== "all" || v !== "all" || p !== "all") && <button type="button" className="cand-btn cand-btn--ghost cand-btn--xs cand-payout-clear-btn" onClick={() => { x("all"); g("all"); m("all"); }}>✕ Clear filters</button>}
</div>
</div>
{d && <div className="cand-modal-error">{d}</div>}
{o ? <div className="cand-exp-empty">Loading…</div> : A.length === 0 ? <div className="cand-exp-empty">No entries match the current filters.</div> : <div className="cand-allexp-table-wrap"><table className="cand-allexp-table"><thead><tr><th>Handler</th><th>Amount (₹)</th><th>Category</th><th>Date</th><th>Note</th><th>Proof</th><th>Actions</th></tr></thead><tbody>{A.map(P => <tr className={`cand-allexp-row cand-allexp-row--payout${_ === P.id ? " cand-allexp-row--editing" : ""}`} key={P.id}><td className="cand-allexp-ref">{P.reference}</td><td className="cand-allexp-amount">{Number(P.amount).toLocaleString("en-IN")}</td><td><span className={`cand-exp-cat cand-exp-cat--${P.category}`}>{Ex[P.category] || P.category}</span></td><td className="cand-allexp-date">{rR(P.date)}</td><td className="cand-allexp-note">{P.note || <em>—</em>}</td><td className="cand-allexp-proof">{(P.proofs && P.proofs.length > 0) ? <button type="button" className="cand-proof-thumb-btn" onClick={() => setPreviewProof(P.proofs[0])} title="View payment proof"><img src={`${ve}${P.proofs[0].url}`} alt="proof" className="cand-proof-thumb-img" loading="lazy" /></button> : <em>—</em>}</td><td className="cand-allexp-actions"><button type="button" className="cand-btn cand-btn--ghost cand-btn--xs" onClick={() => ce(P)} title="Edit">✎</button><button type="button" className="cand-btn cand-btn--ghost cand-btn--xs cand-btn--danger-ghost" onClick={() => B(P)} title="Delete">🗑</button></td></tr>)}</tbody></table></div>}
</div><footer className="cand-modal-footer"><button type="button" className="cand-btn cand-btn--ghost" onClick={r}>Close</button></footer></div></div>{previewProof && <div className="cand-proof-lightbox" onClick={() => setPreviewProof(null)}><div className="cand-proof-lightbox-inner" onClick={P => P.stopPropagation()}><button type="button" className="cand-proof-lightbox-close" onClick={() => setPreviewProof(null)} aria-label="Close preview">×</button><img src={`${ve}${previewProof.url}`} alt={previewProof.note || previewProof.original_name || "Payment proof"} className="cand-proof-lightbox-img" />{previewProof.note && <p className="cand-proof-lightbox-note">{previewProof.note}</p>}<p className="cand-proof-lightbox-meta">{previewProof.original_name}{previewProof.uploaded_at && <span> · {new Date(previewProof.uploaded_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>}</p></div></div>}</s.Fragment>;
}
function At(e) {
  const t = Number(e) || 0;
  if (t) {
