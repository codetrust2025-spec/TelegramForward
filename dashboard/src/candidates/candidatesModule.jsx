...
function _Component29({
  ...
  return <div className="cand-modal-backdrop" onClick={P => P.target === P.currentTarget && (r == null ? undefined : r())}><div className="cand-modal cand-modal--xl cand-payout-modal"><header className="cand-modal-header cand-payout-header"><div className="cand-payout-header-left"><h3 className="cand-modal-title">Manage handler payouts</h3><p className="cand-modal-sub cand-payout-bar"><span className="cand-payout-chunk"><strong>{Y.count}</strong> entr{Y.count === 1 ? "y" : "ies"}</span><span className="cand-payout-chunk cand-payout-chunk--earn" title="Auto-computed: 50% with shortfall penalty when client paid below prescribed tariff"><span className="cand-payout-pip" /> Owed (50%) <strong>{Jc(O)}</strong></span><span className="cand-payout-chunk cand-payout-chunk--ded" title="Every row in the payout ledger"><span className="cand-payout-pip" /> Paid out <strong>{Jc(M)}</strong></span><span className={`cand-payout-chunk ${C > 0 ? "cand-payout-chunk--net-pos" : C === 0 ? "cand-payout-chunk--net-zero" : "cand-payout-chunk--net-neg"}`}><span className="cand-payout-pip" />{C > 0 ? "Still owe " : C === 0 ? "Settled " : "Overpaid by "}<strong>{Jc(Math.abs(C))}</strong></span></p></div><div className="cand-payout-header-right">{(v !== "all" || p !== "all") && <button type="button" className="cand-btn cand-btn--ghost cand-btn--sm" onClick={() => { g("all"); m("all"); }}>Clear filters</button>}<button type="button" className="cand-modal-close" onClick={r} aria-label="Close">×</button></div></header><div className="cand-modal-body cand-modal-body--stack"><div className="cand-payout-content"><form className="cand-allexp-form cand-exp-form--payout cand-payout-form" onSubmit={ee}><label className="cand-field"><span className="cand-field-label">Handler *</span><select className="cand-input" value={k.reference} onChange={P => T(j => ({ ...j, reference: P.target.value }))} required={true}><option value="">Select handler</option>{J.map(P => <option value={P} key={P}>{P}</option>)}</select></label><label className="cand-field"><span className="cand-field-label">Amount (₹) *<span className="cand-exp-kind-tag cand-exp-kind-tag--payout">subtracted from what's owed</span></span><input className="cand-input" type="number" min="0" step="100" value={k.amount} onChange={P => T(j => ({ ...j, amount: P.target.value }))} placeholder="5000" required={true} /></label><label className="cand-field"><span className="cand-field-label">Category</span><select className="cand-input" value={k.category} onChange={P => T(j => ({ ...j, category: P.target.value }))}>{B0.map(P => <option value={P.value} key={P.value}>{P.label}</option>)}</select></label><label className="cand-field"><span className="cand-field-label">Date</span><input className="cand-input" type="date" value={k.date} onChange={P => T(j => ({ ...j, date: P.target.value }))} /></label><label className="cand-field cand-field--span2"><span className="cand-field-label">Note</span><input className="cand-input" value={k.note} onChange={P => T(j => ({ ...j, note: P.target.value }))} placeholder="e.g. May commission · taxi to client meeting" /></label><div className="cand-exp-form-actions cand-allexp-form-actions">{_ && <button type="button" className="cand-btn cand-btn--ghost" onClick={G}>Cancel edit</button>}<button type="submit" className="cand-btn cand-btn--primary" disabled={S}>{S ? "Saving…" : _ ? "Save changes" : "+ Log payout"}</button></div></form><div className="cand-payout-sidebar"><label className="cand-field"><span className="cand-field-label">Period</span><select className="cand-input" value={v} onChange={P => g(P.target.value)} aria-label="Filter by month">{Z.map(P => <option value={P.value} key={P.value}>{P.label}</option>)}</select></label><label className="cand-field"><span className="cand-field-label">Category</span><select className="cand-input" value={p} onChange={P => m(P.target.value)} aria-label="Filter by category"><option value="all">All categories</option>{B0.map(P => <option value={P.value} key={P.value}>{P.label}</option>)}</select></label></div></div>{d && <div className="cand-modal-error">{d}</div>}{o ? <div className="cand-exp-empty">Loading…</div> : A.length === 0 ? <div className="cand-exp-empty">No entries match the current filters. Use the form above to add the first one, or <button type="button" className="cand-link" onClick={() => { g("all"); m("all"); }}>clear filters</button>.</div> : <div className="cand-allexp-table-wrap"><table className="cand-allexp-table"><thead><tr><th>Handler</th><th>Amount</th><th>Category</th><th>Date</th><th>Note</th><th>Actions</th></tr></thead><tbody>{A.map(P => <tr className={`cand-allexp-row cand-allexp-row--payout${_ === P.id ? " cand-allexp-row--editing" : ""}`} key={P.id}><td className="cand-allexp-ref">{P.reference}</td><td className="cand-allexp-amount cand-allexp-amount--payout">−{Jc(P.amount)}</td><td><span className={`cand-exp-cat cand-exp-cat--${P.category}`}>{Ex[P.category] || P.category}</span></td><td className="cand-allexp-date">{rR(P.date)}</td><td className="cand-allexp-note">{P.note || <em>—</em>}</td><td className="cand-allexp-actions"><button type="button" className="cand-btn cand-btn--ghost cand-btn--xs" onClick={() => ce(P)} title="Edit">✎</button><button type="button" className="cand-btn cand-btn--ghost cand-btn--xs cand-btn--danger-ghost" onClick={() => B(P)} title="Delete">🗑</button></td></tr>)}</tbody></table></div>}</div><footer className="cand-modal-footer"><button type="button" className="cand-btn cand-btn--ghost" onClick={r}>Close</button></footer></div></div>;
}
function At(e) {
  const t = Number(e) || 0;
  if (t) {
