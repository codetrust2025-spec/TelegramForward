...
function X8({
  ...
  return <div className="cand-modal-backdrop" onClick={C => C.target === C.currentTarget && (t == null ? undefined : t())}><form className="cand-modal" onSubmit={M}><header className="cand-modal-header"><h3 className="cand-modal-title">{e ? "Edit candidate" : "Add candidate"}</h3><button type="button" className="cand-modal-close" onClick={t} aria-label="Close">×</button></header><div className="cand-modal-body"><label className="cand-field cand-field--span2"><span className="cand-field-label">Candidate name *</span><input ref={v} className="cand-input" value={l.name} onChange={C => g("name", C.target.value)} placeholder="e.g. NIKHIL" required={true} /></label><label className="cand-field"><span className="cand-field-label">Stage</span><select className="cand-input" value={l.stage} onChange={C => g("stage", C.target.value)}>{V8.map(C => <option value={C.value} key={C.value}>{C.label}</option>)}</select></label><label className="cand-field cand-field--span2"><span className="cand-field-label">Technology</span><input className="cand-input" value={l.technology} onChange={C => g("technology", C.target.value)} placeholder="e.g. SAP BASIS, React JS, AWS Admin…" list="cand-tech-list" /><datalist id="cand-tech-list">{H8.map(C => <option value={C} key={C} />)}</datalist></label><label className="cand-field"><span className="cand-field-label">Phone{l.phone && <span className="cand-field-aside"><_Component23 phone={l.phone} inline={true} /></span>}</span><input className="cand-input" type="tel" value={l.phone} onChange={C => g("phone", C.target.value.replace(/[^\d+]/g, ""))} placeholder="9876543210" /></label><label className="cand-field"><span className="cand-field-label">Reference</span><ReferencePicker value={l.reference} onChange={C => g("reference", C)} options={refOpts} readOnly={a} placeholder="Handler name" title={a ? "Your handler name is set automatically" : undefined} /><span className="cand-field-hint">Earns 50% commission.</span></label><div className="cand-field cand-field--span2 cand-service-field"><span className="cand-field-label">Service type</span><div className="cand-service-options"><label className={`cand-service-card${l.service_type === "profile_service" ? " cand-service-card--on" : ""}`}><input type="radio" name="cand-service-type" checked={l.service_type === "profile_service"} onChange={() => m("profile_service")} /><span className="cand-service-card-body"><strong>Profile</strong><em>₹{Cu.toLocaleString("en-IN")}</em></span></label><label className={`cand-service-card${l.service_type === "round_wise" ? " cand-service-card--on" : ""}`}><input type="radio" name="cand-service-type" checked={l.service_type === "round_wise"} onChange={() => m("round_wise")} /><span className="cand-service-card-body"><strong>Round-wise</strong><em>₹{wi.toLocaleString("en-IN")}/₹{ki.toLocaleString("en-IN")}</em></span></label></div></div>{l.service_type === "round_wise" && <div className="cand-field cand-field--span2 cand-service-scope"><span className="cand-field-label">Scope</span><div className="cand-service-scope-options"><label className={`cand-service-scope-pill${!Eu(l.interview_scope) ? " cand-service-scope-pill--on" : ""}`}><input type="radio" name="cand-interview-scope" checked={!Eu(l.interview_scope)} onChange={() => _("external")} />External · ₹{wi.toLocaleString("en-IN")}</label><label className={`cand-service-scope-pill${Eu(l.interview_scope) ? " cand-service-scope-pill--on" : ""}`}><input type="radio" name="cand-interview-scope" checked={Eu(l.interview_scope)} onChange={() => _("internal")} />Internal · ₹{ki.toLocaleString("en-IN")}</label></div></div>}{l.service_type === "profile_service" && <label className={`cand-field cand-consultancy-field${l.consultancy ? " cand-consultancy-field--on" : ""}`}><span className="cand-field-label">Channel</span><div className="cand-consultancy-toggle"><input type="checkbox" id="cand-consultancy-cb" checked={!!l.consultancy} onChange={C => y(C.target.checked)} /><label htmlFor="cand-consultancy-cb" className="cand-consultancy-label"><span className="cand-consultancy-pip" aria-hidden={true} /><span className="cand-consultancy-text"><strong>{l.consultancy ? "Consultancy" : "Direct"}</strong><em>₹{N_(l.consultancy).toLocaleString("en-IN")}</em></span></label></div></label>}<label className="cand-field"><span className="cand-field-label">Expected ₹</span><input className="cand-input" type="number" min="0" step="500" value={l.expected_payment} onChange={C => g("expected_payment", C.target.value)} placeholder={String(os(l.service_type, l.consultancy, l.interview_scope))} /></label><label className="cand-field"><span className="cand-field-label">Received ₹</span><input className="cand-input" type="number" min="0" step="500" value={l.payment} onChange={C => g("payment", C.target.value)} placeholder="0" /><span className={`cand-pay-status cand-pay-status--${E}`}>{E === "paid" && <s.Fragment>✓ Paid ({$n(k)})</s.Fragment>}{E === "partial" && <s.Fragment>● {$n(k)}/{$n(T)} · <strong>{$n(S)} pending</strong></s.Fragment>}{E === "unpaid" && <s.Fragment>○ {$n(T)} pending</s.Fragment>}</span>{k > 0 && (l.reference || "").trim() && <span className="cand-pay-handler-share">↻ {l.reference.trim()} earns {$n(wl(k, T, P))}</span>}</label><label className="cand-field"><span className="cand-field-label">Date</span><input className="cand-input" type="date" value={l.date} onChange={C => g("date", C.target.value)} /></label>{b && <label className="cand-field cand-field--span2 cand-field--required"><span className="cand-field-label">Follow-up<span className="cand-field-required-tag">balance {$n(S)}</span></span><textarea className="cand-input cand-input--textarea" rows={2} value={l.follow_up} onChange={C => g("follow_up", C.target.value)} placeholder="Why is balance pending? When will it be paid?" required={true} /></label>}<div className="cand-field cand-field--span2"><$8 candidateId={e == null ? undefined : e.id} proofs={o} onChange={u} /></div></div>{h && <div className="cand-modal-error">{h}</div>}<footer className="cand-modal-footer"><button type="button" className="cand-btn cand-btn--ghost" onClick={t} disabled={d}>Cancel</button><button type="submit" className="cand-btn cand-btn--primary" disabled={d}>{d ? "Saving…" : e ? "Save changes" : "Add candidate"}</button></footer></form></div>;
}
function cr(e) {
  const t = Number(e) || 0;
  if (t === 0) {
