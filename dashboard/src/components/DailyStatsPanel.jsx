...
export function DailyStatsPanel({
  ...
  const campaignPosts = scoped
    ? (scoped.campaign_posts ?? 0)
    : sumScopedStats(dailyStats, accountSlots, accountStates, null, r => r.campaign_posts ?? 0)
  ...
  const campaignPostsInMode = scopeAccount
    ...
    : sumByPostingMode(dailyStats, accountSlots, accountStates, postingModes, 'campaign', r => r.campaign_posts ?? 0)
  ...
  const ranked = [...accountSlots]
    ...
    .sort((a, b) => {
      const score = row => {
        const posts = (row.campaign_posts ?? 0) + (row.forward_posts ?? 0)
        if (reachView === 'forwarding') return row.forward_posts ?? 0
        if (reachView === 'campaign') {
          return (row.campaign_posts ?? 0)
            + (row.contacts || 0)
            + (row.incoming || 0)
            + (row.outgoing || 0)
        }
  ...
  const statsTitle = scopeAccount
    ...
    : accountsModeFilter === 'forwarding'
      ? 'Fleet today reach · forwarding'
      : accountsModeFilter === 'campaign'
        ? 'Fleet today reach · campaign'
        : 'Fleet today reach'
  /* Setup filter only locks fleet-wide reach; per-account view keeps All / Campaign / Forwarding tabs */
  ...
  return (
    <section className="daily-stats-panel" aria-label={scopeAccount ? 'Account today reach' : 'Fleet today reach and daily counters'}>
      ...
      {reachView === 'all' ? (
        <>
          ...
          <MetricGrid columns={4} className="daily-stats-grid">
            <MetricBlock label="Contacts" value={contacts} tone="success" title="Unique DM contacts in this window (all accounts)" />
            ...
            <MetricBlock
              ...
              title="Successful master-list cycle posts since reset (fleet)"
            />
            <MetricBlock
              ...
              title="Successful interval forwards since midnight IST — cumulative, not this tick"
            />
            <MetricBlock
                ...
                <li key={row.slot} className="daily-stats-account-row">
                  ...
                  <span className="daily-stats-account-metrics">
                    {reachView === 'forwarding'
                      ...
                      : reachView === 'campaign'
                        ? `${row.contacts ?? 0} contacts · ${row.incoming ?? 0} in · ${row.outgoing ?? 0} out · ${row.campaign_posts ?? 0} posts`
                        : `${row.contacts ?? 0} contacts · ${row.campaign_posts ?? 0} cycle · ${row.forward_posts ?? 0} forward`}
                  </span>
                  <button
                    type="button"
      ...
      <p className="daily-stats-footnote">
        {reachView === 'all'
          ? 'All combines fleet DM stats with campaign-cycle and forwarding post counts. Use Campaign cycles or Forwarding tabs for mode-specific live counters.'
          : reachView === 'forwarding'
        ...
        {scopeAccount
          ...
          : 'Counters follow the IST calendar day (auto-reset at midnight IST). Manual reset clears from now until midnight IST. Accounts, chats, and logs are kept.'}
      </p>
