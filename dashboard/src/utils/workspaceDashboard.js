...
/** True when this account is running or sleeping for the current workspace mode. */
export function isAccountProcessActiveForModeFilter(
  acct,
  slot,
  state,
  postingModes,
  ...
  if (modeFilter === 'forwarding') {
    ...
    return !!fwd.running || fwd.status === 'sleeping' || status === 'running' || status === 'sleeping'
  }
  if (modeFilter === 'campaign') {
    ...
    return !!camp.running || camp.status === 'sleeping' || status === 'running' || status === 'sleeping'
  }
...
export function isAccountRunningForWorkspace(acct, slot, state, postingModes, workspaceMode) {
  return isAccountProcessActiveForModeFilter(
    acct,
    slot,
    state,
    postingModes,
...
export function fleetWorkspaceAnyRunning(loggedInSlots, state, postingModes, workspaceMode, fleet) {
  ...
  if (
    slots.some(slot =>
      isAccountProcessActiveForModeFilter(
...
export function fleetWorkspaceCanStartMore(loggedInSlots, state, postingModes, workspaceMode) {
  ...
  if (!slots.length) {
    return activeFleetSlots(state, loggedInSlots).some(
      slot => !state.account_states?.[slot]?.running,
    )
  }
  return slots.some(
    slot =>
