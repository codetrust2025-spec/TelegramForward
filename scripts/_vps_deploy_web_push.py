"""Deploy Web Push (backend routes + dashboard + sw.js + pywebpush)."""
...
BUILD_STAMP = "2026-06-05-web-push"
...
def main() -> int:
    ...
    source_files = [
        ...
        "dashboard/src/utils/webPush.js",
        "dashboard/src/components/WebPushToggle.jsx",
    ...
    pip_cmd = (
        ...
        "'pywebpush>=2.0.0' 'py-vapid>=1.9.0' 2>&1 | tail -3"
    )
    _, o, _ = client.exec_command(pip_cmd, timeout=180)
    print("\npip:", o.read().decode().strip())

    _, o, _ = client.exec_command(
        f"grep -o '{BUILD_STAMP}' /opt/telegramforward/static/assets/app-*.js | head -1; "
        "grep -c install_web_push_routes /opt/telegramforward/server.py; "
        "test -f /opt/telegramforward/static/sw.js && echo sw.js:ok; "
        "curl -s http://127.0.0.1:8000/push/vapid-public-key | head -c 80",
        timeout=30,
    )
