...
# Initials / abbreviations — not greeting names (R.B., A.K., RB, etc.)
...
def _is_initials_or_abbrev(name: str) -> bool:
    """True for R.B., A.K., RB, etc. — not a speakable first name."""
    s = (name or "").strip()
    if not s:
        return True
    ...
    if _INITIALS_NAME_RE.match(s):
        return True
    # Two–three letter all-caps tokens (RB, AK) without vowel-heavy word shape
...
def _normalize_first_name_token(token: str) -> str:
    """Title-case a single name token if it looks like a real given name."""
...
def _first_name(lead: dict | None) -> str:
    """Client first name from lead/inbox — empty if unknown or not a real name."""
    ...
    if not parts:
        return ""
    # Use the first token that looks like a real given name (skip R.B., A.K., etc.)
    for part in parts:
...
def _user_wants_full_placement_from_scratch(
    user_text: str,
    history: list[dict] | None,
) -> bool:
    """End-to-end job help (resume → calls → placement) — not a single interview round."""
    blob = _user_only_blob(history or [], user_text, tail=24).lower()
    if any(m in blob for m in _FROM_SCRATCH_PLACEMENT_MARKERS):
        return True
    if "scratch" in blob and any(
        w in blob for w in ("job", "work", "project", "everything", "done", "need", "support", "nundi")
    ):
        return True
    return False


def _thread_has_scheduled_interview(
    history: list[dict] | None,
    user_text: str = "",
) -> bool:
    blob = _conversation_blob(history or [], user_text, tail=24).lower()
    if any(m in blob for m in _INTERVIEW_OR_EXAM_SCHEDULED_MARKERS):
        return True
...
def _full_placement_from_scratch_reply(
    user_text: str,
    *,
    history: list[dict] | None,
    lang: str = "english",
    ...
    if en:
        return (
            f"Got it {addr}👍 you want full support from scratch — resume, profile, and job search. "
            f"Are you a fresher or do you have IT experience? Which tech are you targeting?"
        )
    return (
        f"Got it {addr}👍 scratch nunchi full support kavali — resume, profile, job search. "
        f"Fresher na IT experience undha? Eme tech target chestunnaru?"
    )
...
def _job_seeker_reply(
    ...
    *,
    ...
    if _user_wants_abroad_job_placement(user_text, history) or _user_wants_abroad_job_placement("", history):
        return _abroad_job_policy_reply(user_text, history=history, lang=lang, lead=lead)
    addr = _addr_prefix(lead, lang)
    if _user_wants_full_placement_from_scratch(user_text, history) or _user_wants_full_placement_from_scratch("", history):
        return _full_placement_from_scratch_reply(
            user_text, history=history, lang=lang, lead=lead,
        )
    has_interview = _thread_has_scheduled_interview(history, user_text)
    wants_interview = _user_wants_interview_support(user_text, history)
    if not has_interview and not wants_interview:
        prefix = f"Got it {addr}👍 " if addr.strip() else "Got it 👍 "
        return _job_search_service_reply(
            history,
            user_text,
            lang=lang,
...
_FROM_SCRATCH_PLACEMENT_MARKERS = (
    "from scratch", "by scratch", "start from scratch", "starting from scratch",
    "everything from scratch", "done by scratch", "should be done by scratch",
    "everything should be done", "end to end", "end-to-end", "start to finish",
    "resume to offer", "offer letter", "full support", "complete support",
    "whole process", "zero to job", "starting fresh", "fresh start",
    "nadhi from scratch", "from scratch nundi",
)
...
def _user_wants_job_search_support(user_text: str, history: list[dict] | None) -> bool:
    ...
    if intent_blocks_job_search(intent):
        return False
    if _user_wants_full_placement_from_scratch(user_text, history):
        return True
    if _user_wants_interview_support(user_text, history):
        return False
...
def _enforce_job_search_service(
    ...
    *,
    ...
    wrong = any(x in lower for x in (
        ...
        "scheduled round", "when you have a scheduled",
    ))
...
def _humanize_reply(
    ...
    *,
    ...
    if (
        _user_wants_job_search_support(user_text, history)
        or _user_wants_job_search_support("", history)
    ):
        addr = _addr_prefix(lead, lang)
        prefix = f"Got it {addr}👍 " if addr.strip() else "Got it 👍 "
        return _fin(_job_search_service_reply(
            history,
            user_text,
            lang=lang,
            include_price=_should_quote_price(user_text, history),
