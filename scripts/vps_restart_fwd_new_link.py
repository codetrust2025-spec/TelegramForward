...
from core.posting_mode import set_posting_mode, set_forwarding_source, load_posting_mode
...
for slot in fwd:
    ...
    set_forwarding_source(slot, source_peer="@jobsupport0", source_message_id={MSG_ID})
    call("POST", f"/account/{{slot}}/start?feature=forwarding")
    pm = load_posting_mode(slot)
    print(f"{{slot}}: started, source=@jobsupport0/{MSG_ID}")
    time.sleep(2)
print("OK")
'''
