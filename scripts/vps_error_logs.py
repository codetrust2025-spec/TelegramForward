...
for slot in ["account3","account5","account7","account8","account10"]:
    logs = st["account_states"].get(slot,{}).get("logs") or []
    print(f"\n=== {slot} errors/warnings ===")
    for lg in logs:
        s = lg if isinstance(lg,str) else (lg.get("msg") or lg.get("message") or json.dumps(lg))
        sl = s.lower()
        if any(k in sl for k in ["error", "fail", "send_fail", "locked", "exception", "health", "get_me", "connection", "appledeveloper"]):
            print(s[:200])
'''
