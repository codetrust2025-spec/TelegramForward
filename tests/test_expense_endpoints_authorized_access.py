...
GET/POST /forward-message/settings proved easy to verify one way: an anonymous
...
def client(monkeypatch):
    ...
    # Keep real expense and forwarding data out of the test entirely.
    ...
    monkeypatch.setattr(
        forward_message_service, "get_settings", lambda: {}, raising=False
    )
...
def test_writing_forwarding_settings_still_works_when_signed_in(client, monkeypatch):
    from services.forward_message_service import forward_message_service
    ...
    monkeypatch.setattr(
        forward_message_service, "save_settings", fake_save, raising=False
    )

    anonymous = client.post("/forward-message/settings", json={"mode": "x"})
    assert anonymous.status_code in (401, 403)
    assert saved == {}, "an anonymous caller must not reach the writer at all"

    allowed = _signed_in(client).post("/forward-message/settings", json={"mode": "x"})
