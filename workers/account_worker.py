...
class AccountWorker:
    ...
    async def _run_forwarding_forever(self) -> None:
        ...
        while acct.running and acct.forwarding_running:
            ...
            try:
                disk_invalid, disk_blocked = load_account_dead(self.slot)
                acct.invalid_groups = disk_invalid
                acct.blocked_groups = disk_blocked
            except Exception:
                pass
            st.success = 0
            st.failed = 0
            ...
            async def _on_target_done(
                ...
                *,
                ...
                if tick_outcome == "sent":
                    st.success += 1
                    label = (st.current_group or "").strip()
                    if label:
                        st.success_list.append(label)
                    try:
                        from core.group_send_stats import record_group_send

                        if label:
                            record_group_send(self.slot, label)
                    except Exception:
                        pass
            ...
            if tick_result.forwarded > 0:
                try:
                    from core.send_stats import record_send_batch

                    record_send_batch(self.slot, int(tick_result.forwarded), "forward")
                except Exception:
                    pass
