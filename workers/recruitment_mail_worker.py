...
class RecruitmentMailWorker:
    def __init__(self):
        ...
        self._watch_task=None;self._ai_recovery_task=None;self._audit_task=None;self._audit_ai_task=None
        ...
        self._last_outcome_audit=float('-inf');self._last_audit_ai=float('-inf')
    def start(self):
        ...
        while not self._stopping:
            try:
                ...
                if os.getenv('AI_INTERVIEW_OFFER_TRACKING_ENABLED', 'true').lower() != 'false':
                    ...
                    # Audit maintenance is the lowest-priority work in this
                    # loop and is scheduled last, only when no Gmail sync job
                    # is in flight.  Live interview processing therefore always
                    # holds the queue slots and the Ollama node ahead of it.
                    if self._audit_task and self._audit_task.done():self._audit_task=None
                    if self._audit_ai_task and self._audit_ai_task.done():self._audit_ai_task=None
                    live_idle=not self._jobs
                    interval=max(300,int(os.getenv('AI_MAIL_AUDIT_INTERVAL_SECONDS','1800')))
                    if live_idle and self._audit_task is None and now-self._last_outcome_audit>=interval:
                    ...
                    ai_interval=max(60,int(os.getenv('AI_MAIL_AUDIT_AI_INTERVAL_SECONDS','300')))
                    if live_idle and self._audit_ai_task is None and now-self._last_audit_ai>=ai_interval:
                        self._last_audit_ai=now
                        self._audit_ai_task=asyncio.create_task(self._run_maintenance('audit-ai',self.process_audit_ai),name='mailbox-audit-ai')
                    active={*self._jobs,*(task for task in (self._watch_task,self._ai_recovery_task,self._audit_task,self._audit_ai_task) if task)}
                    if active:
    ...
    def process_audit_ai(self):
        """Lowest-priority Ollama work: a second opinion on audit findings.

        Isolated from interview auto-booking by construction — it imports no
        booking module, writes only to the audit's own tables, and its own
        gate defers whenever a Gmail sync job, an AI backlog item or an
        ingestion row exists.  A failure here is logged and dropped; it can
        ...
        try:
            ...
            logger.exception('Audit second-opinion pass failed; booking is unaffected')
            return
        if result.get('deferred'):
            logger.debug('Audit AI yielded to live processing: %s',result.get('reason'))
        elif result.get('ran'):
            logger.info('Audit second opinions completed=%s failed=%s',
              result.get('ran'),result.get('failed'))
