    ...
    *,
    ...
    for row in creds.get("handlers") or []:
        if str(row.get("username") or "").strip().lower() == str(username or "").strip().lower():
            ...
            break
    site = (creds.get("site_url") or "https://teleautomation.online").strip()
    out = {
        "site_url": site,
        "submit_slot_url": f"{site.rstrip('/')}/submit-slot",
        "username": username,
...
def save_credentials(
    *,
    ...
    data = {
        "site_url": (site_url or "https://teleautomation.online").strip(),
